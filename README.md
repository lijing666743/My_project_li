...
- Oracle-1 Infrastructure V1 已实现：补齐 `FACTUAL_ROUTE_BRANCH` 显式 validity/reuse sidecar 字段，严格区分 `FACTUAL_ROUTE`、`BEST_COMMITTED_ROUTE` 与 `ORACLE1_BEST_ROUTE`；factual Local/Remote 复用对应 committed branch，factual Defer 使用同初始状态/RNG/当前 factual joint action/frozen downstream policy 的独立 Defer shadow；冻结 runner 的数学 identity 固定为 `(policy_version, actor_state_digest)`，provenance 独立记录；fingerprint 改为确定性 canonical fail-fast，`clone_for_shadow()` 增加结构和 mutation isolation gate，并补齐 CRN、terminal/time、energy attribution、policy-miss、resume identity、checkpoint state、OFF/ON behavior neutrality 与 selected-decision-first 三重预算测试。最新 full regression 共 637 项：632 passed、0 failed、5 项 CUDA-only skipped。Oracle feasibility 尚未运行，root cause 尚未分类，Oracle-2 尚未实施，60K/500K 尚未批准；当前不构成正式 Oracle diagnosis、Remote advantage、root cause 或正式实验结论。
- Actor-Only Oracle Diagnostic Carrier V1 已实现：公开 `ActorOnlyRouteOracleRunner` 以严格 `FINAL_COMPLETED` checkpoint actor-only source、冻结 learned actor、production factual sampling/RNG/GRU 路径和现有 Oracle-1 shadow infrastructure 执行独立预算诊断；carrier 不构造 critic、rollout、GAE、PPO updater、optimizer、checkpoint save 或 training resume，允许 episode 中途按 factual cap 正常结束。独立 identity 使用 current Git commit 与 tracked working-tree diff digest，并显式排除 protected PDFs；source/current config hash 无需相同，只执行既有 environment/observation/action/actor 语义兼容检查。新增 carrier targeted tests 10/10 PASS；最新 full regression 共 647 项：642 passed、0 failed、5 项 CUDA-only skipped。本轮未运行真实 4K/60K Oracle pilot、未启动训练、未生成 post-collapse 结果，等待人工审核。

- Same-State Zero-Hidden Route Probe V1 diagnostic infrastructure 已实现：严格绑定 4K/60K 各自 checkpoint SHA-256 与 actor state digest，并拒绝同 checkpoint、同 digest、身份交叉或标签错配；目标集合固定为两个 pilot 各 16 个 decision，缺失、重复、额外记录或 sidecar/route join 不完整均失败关闭。runner 在 source-policy factual replay 的同一 pre-step state 上，以同一 actor tensor batch 和各 actor 独立全零 hidden 比较 production deterministic route distribution；逐 decision 验证 state/RNG/exogenous/hidden/factual route/task fingerprint，并保护 factual policy generator、Torch CPU/CUDA RNG、environment RNG、factual hidden、actor 参数和输入张量不被 probe 推进或污染。只有 32/32 decision 全部完成且 logits/probabilities 有限、预算未耗尽时才原子发布成功 sidecar；Oracle-1 注释仅作为历史 source-policy 标签保留，不执行 shadow、不重分类 root cause。新增专项测试 14/14、相关 Oracle/route 回归 61/61、full unittest 659/659 PASS（0 failure、0 error、0 skip，CUDA RNG 门禁已实际执行）。本轮未运行真实 probe、4000/60000/500K training 或正式 root-cause classification，未生成实验结论、未 stage、未 commit。
...
- Route-Specific Entropy Schedule Parameterization 已完成实现与门禁验证：复用既有开关并严格限定为 route-only；global/base 及其他六分支 entropy coefficient 保持 0.01；route coefficient 在 `t < 3072` 为 0.01，`t = 3072` 有意跳变为 0.03，随后按 collected environment transitions 线性退火并在 `t >= 32768` 回到 0.01。schedule 由 resolved config 与 `collected_environment_transitions` 纯重建，Checkpoint V1 无新增 mutable state，diagnostics schema v4 追加六个最小字段并保持旧 CSV/checkpoint reader 兼容；targeted 268/268（82 subtests）、相邻回归 44/44（37 subtests）、full regression 512/512（140 subtests）PASS。本轮未训练、未运行 60k/Seeds 43–46/500k、未生成新训练 artifact、未提交 commit，等待人工审核。
- Minimal Task-Candidate Diagnostic Instrumentation V1 已实现并通过 actor-safe、sidecar isolation、local/remote/defer coverage、utility reconstruction、OFF/ON neutrality、RNG、PPO 与 checkpoint training-state gates；新增独立 `route_diagnostic_samples_v1.jsonl`，utility 由离线纯函数重建。targeted tests 与 full unittest 613/613 PASS（5 项 CUDA 条件跳过）；本轮未进行 4000 diagnostic collection、60K/500K 或正式训练，等待人工审核。
- Same-State Zero-Hidden Route Probe V1 provenance 最小修复已完成：`diagnostic_config()` 保留 `RunConfig` 的 `git_commit`、`git_branch` 与 `git_dirty`，未重新读取 Git 或绕过 actor-loader 检查；专项 probe 13/14（1 项跳过）、runner 1/1、actor loader/Oracle 41/41、full unittest 662/662（6 项跳过）通过。本轮未运行真实 probe、未训练、未 stage、未 commit。
- Same-State Zero-Hidden Route Probe V1 execution-device fail-fast 已完成：runner 在 actor/checkpoint 加载和 factual replay 前读取 4K/60K pilot header 的 `oracle_diagnostic_identity.execution_device`，严格要求两者与 runner device 三方一致；缺失、非法或不一致均失败关闭并报告三个设备值，未修改 fingerprint、hidden digest、probe contract、模型、环境或训练。runner 定向测试 2/2、Same-State/Oracle 相关回归 44/44（1 项 CUDA-only 跳过）、full unittest 663/663（6 项 CUDA-only 跳过）通过；本轮未运行真实 probe 或正式训练，真实 pinned pilots 仍必须使用 CUDA runner。
- Same-State Zero-Hidden Route Probe V1 CUDA RNG isolation 最小修复已完成：`CAGATMAPPOActor` 的 CPU 参数初始化仅设置 Torch CPU default generator，不再通过 `torch.manual_seed` 重置 global CUDA RNG；actor architecture、state_dict、checkpoint、loader、probe contract、fingerprint、hidden digest、environment、PPO、reward 与 observation 均未修改。新增 CUDA-initialized actor construction 与真实临时 FINAL checkpoint `_load_actor_rng_isolated(..., "cuda")` 回归，两个 CUDA 用例均实际执行；专项 probe 16/16、runner 2/2、actor-only Oracle 10/10、route Oracle diagnostic 18/18、full unittest 665/665 PASS（0 failure、0 error、0 skip）。本轮未运行真实 Same-State Probe、未训练、未生成实验产物、未 stage、未 commit；`READY_FOR_REAL_PROBE_RUN=YES`。
- Route Decoder Localization V1 离线诊断基础设施已实现：只读取既有 Same-State sidecar 与其严格绑定的 4K/60K FINAL actor，按 A（encoder/GAT/GRU）、B（candidate/edge/query representation）和 C（fixed head/remote scorer/final route logits）执行参数差异与临时 forward-hook 激活审计；正式 sidecar 的 32 个 decision 对应 29 个去重 state，全部 state 均包含 values/shape/dtype/SHA-256 完整 actor input tensor。专项与相邻回归 38 项通过、3 项因本机无 CUDA 跳过；本轮未运行 localization、未训练、未修改模型/environment/reward/PPO、未读取 protected PDFs、未 stage、未 commit；`READY_FOR_LOCALIZATION_RUN=YES`。
- Route Decoder Localization V1 已在既有 29 个去重 same-state tensor / 32 个 paired decisions 上完成只读运行，严格复现 4K/60K 概率、entropy 与 32/32 Remote→Local argmax 基线。post-hook 激活差异按 median relative L2 排名为 C（1.996620）> B（0.983981）> A（0.576335）：A 的 GRU 已出现中等漂移，B 的 query/candidate projection 接近正交，remote scorer 输入在 29/29 状态转为负 cosine，C 的 remote scorer/final route logits 在 29/29 状态近反向；max-remote-minus-local margin 均值由 4K 的 1.850067 变为 60K 的 -5.589151。localization 结论为 `Mixed`，主要证据链为 B decoder representation drift → C terminal scorer amplification，A 为上游次要贡献；该结果不是训练因果证明。本轮未训练、未修改模型/reward/PPO/GAE/environment、未读取 protected PDFs、未 stage、未 commit；`READY_FOR_FIX_DESIGN=YES`。
- Route Preference Collapse Fix V1 / Candidate A 基础设施已实现：在 PPO backward 前接入仅对 route-active、Local legal 且至少一个 Remote legal 状态生效的 conditional Remote–Local entropy-floor stability loss（`H_min=0.20 nats`）；非法 Remote 不参与聚合或梯度。默认关闭及系数为零均保持 legacy loss/计算图身份，actor architecture、action sampling、reward、GAE、environment、checkpoint schema 与 resume contract 均未修改；专项 stability 14/14、historical config 10/10、PPO/update 18/18、candidate-aware decoder 12/12、Same-State 18/18、Localization 8/8、full unittest 687/687 PASS（0 skip、0 failure、0 error）。本轮未运行真实训练、short training、新 Same-State Probe 或新 Localization；合法的 pre-collapse full-resume checkpoint 不可用，因此等待人工审核且 `READY_FOR_SHORT_TRAINING_RUN=NO`。
- Matched Short Training Infrastructure V1 与 Budget Alignment V1 已实现：from-scratch triplet 继续使用配置/actor/critic/optimizer/RNG 身份门禁、live JSONL telemetry、stop evaluator、matched analyzer 与 checkpoint-plan fail-fast；预算协议已由不满足 episode boundary 的 32768/40960 修正为 smoke=1500（checkpoint interval=500）、diagnostic=32000（interval=4000）及仅经人工批准后另起 from-scratch triplet 的 conditional fresh extension=42500（interval=4000）。32000 是 episode horizon=500 与 PPO rollout capacity=256 的 joint lossless boundary，共 64 episodes、125 次完整 rollout、32000 optimized transitions；此时既有 route-entropy schedule 尚未完全结束，route coefficient 约为 0.01051724，未修改 schedule。42500 合法生成 `FINAL_COMPLETED`，但仅 42496 transitions 进入 PPO optimization，末尾 4 transitions 记录为 `unused_final_tail_transitions` 后清空，禁止描述为 32K continuation、禁止 resume 或拼接日志。专项 41/41、完整 unittest 728/728 PASS；本轮尚未运行 Stage-0 preflight、smoke、matched training、Same-State Probe 或 Localization，也未生成真实训练结果，Candidate A 的有效性尚无实验结论。
- Stage 0 Manifest Contract Fix V1 已完成：immutable preflight manifest 现一次性包含 timezone-aware ISO-8601 `timestamp`、显式 `group_definitions`、smoke/diagnostic/extension 完整 `budget_plan`、以及 fail-fast 的 `output_collision_result`；新增 required-field validator 与新/空/非空输出目录门禁，未改变 config/actor/critic/optimizer/RNG/stability identity 逻辑。专项测试 46/46、相邻 targeted 171/171、完整 unittest 733/733 PASS，Stage 0 尚未重新执行，smoke/training 尚未执行，Candidate A 有效性仍未知。
- Stop Evaluator V2 已完成：移除 frequency-only persistent gradient clipping hard stop，保留 `gradient_clip_fraction`、`gradient_clip_triggered` 与 pre-clip norm telemetry；NaN/Inf、KL、PPO clip fraction 及 runtime/artifact fail-closed gates 保持不变。未改变训练超参数（`gradient_clip_norm=0.5`）、Candidate A、PPO、checkpoint schema 或 resume contract；本修改后尚未重新执行 Stage 0、Stage 1 smoke 或 32K diagnostic training。
- Hierarchical Option-Aware V1 evaluation route telemetry 已实现：Formal Evaluation 复用同一次 deterministic action-distribution 输出，以只读旁路记录 top-level 概率、条件 Remote 目的地概率、flat route index、route mask、既有 TX/resource/power proposal 与执行结果，并在独立 `logs/diagnostics/hierarchical-route-eval__<evaluation_id>/` 写出 `summary.json` 和 `route_decision_trace.jsonl`，既有四个 formal artifact 的 schema 与字段含义不变。三种 decoder 兼容及 action-neutrality 定向测试 3/3、Formal Evaluation 19/19、action distribution 11/11 PASS；指定 32K FINAL checkpoint 的真实 CUDA deterministic evaluation 复现旧 run 的 20/20 episode metrics 与 action trace hash，493 个 route-active 决策为 Local=493、Remote=0、Defer=0，checkpoint SHA-256 保持不变。本轮未训练、未修改模型/reward/environment/PPO/rollout/checkpoint、未提交 commit。
- Hierarchical Option-Aware V1 Entropy Ablation Phase 0 infrastructure 已完成：既有 route-only schedule 算法与 `3072–32768` 边界保持不变，启用时的 peak coefficient 由固定 `0.03` 参数化为受控集合 `{0.03, 0.05}`，global/base 及其他六个 action branch entropy coefficient 继续保持 `0.01`；新增 B0（schedule off）、A（peak 0.03）与 B（peak 0.05）三份 small/seed-42/32K matched config，并验证 resolved config 除 entropy treatment 字段外完全一致。专项测试 13/13、相邻 config/PPO/update 回归 86/86 PASS；本轮未训练、未修改 checkpoint/decoder/environment/reward/PPO objective/action space、未提交 commit。
