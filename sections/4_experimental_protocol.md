...
| 区域 bounds | x,y in [0,1000] m | Section 2 的 1000 m × 1000 m 轴对齐区域 |
...
| UAV 初始速度均值 v_bar | (0,0) m/s | IMPLEMENTATION DEFAULT — Section 4 |
| UAV 速度创新标准差 | (1,1) m/s | IMPLEMENTATION DEFAULT — Section 4，逐坐标 Gaussian |
| Gauss-Markov coefficient alpha | 0.85 | Section 2 编码初始值 |
| candidate neighbor radius R_cand | 500 m | Section 2 编码初始值 |
| total bandwidth B_tot | 20 MHz | Section 2 参数表；不得改写 |
| RU count R | 20 | Section 2 参数表；不得改写 |
| RU bandwidth B_RU | 1 MHz | 由 B_tot/R 唯一得到 |
...
| reference transmit power P_ref | 1 W | Section 2 编码初始值 |
| reference CPU frequency f_ref | 2.0e9 cycle/s | Section 2 编码初始值 |
| reference CPU coefficient kappa_ref | 1.0e-28 | Section 2 编码初始值 |
| reference initial energy E_ref0 | 30 J | Section 2 编码初始值 |
| reference rate R_ref | 20e6 bit/s | Section 2 编码初始值 |
| antenna gain | 2 dBi at both ends | Section 2 编码初始值 |
| noise PSD N0 | -174 dBm/Hz, converted to linear W/Hz | IMPLEMENTATION DEFAULT — Section 4 |
| receiver noise figure F_dB | 7 dB | Section 2 编码初始值 |
| building loss L_B | 15 dB when blocked, 0 dB otherwise | Section 2 主模型编码初始值 |
| LoS Rician K_dB | 6 dB | Section 2 已给出 |
| shadowing std sigma_s | 4 dB | Section 2 编码初始值 |
| shadowing correlation distance d_corr | 50 m | Section 2 编码初始值 |
| fixed CSI AoI | 3 slot | IMPLEMENTATION DEFAULT — Section 4；whole episode constant |
| CSI error std sigma_CSI | 2 dB | IMPLEMENTATION DEFAULT — Section 4 |
| interference EMA beta_I | 0.8 | IMPLEMENTATION DEFAULT — Section 4 |
| initial interference value I_def | 0 W with availability mask 0 | IMPLEMENTATION DEFAULT — Section 4；不是有效零测量 |
...
F=10^(F_dB/10)，P_noise,RU=N0*B_RU*F。所有噪声、功率、SINR 和 rate 计算在同一功率域中完成；不得将 dB 值直接代入线性公式。
...
每个 UAV 的资源参数由 Section 2 的 profile 比例乘以参考值，再使用独立、可复现的逐参数扰动 delta_i,m ~ Uniform(-0.05,0.05) 生成。五类 profile 的比例固定如下：

| profile | f_max/f_ref | P_max/P_ref | E0/E_ref0 | kappa/kappa_ref |
|---|---:|---:|---:|---:|
...
- D_n ~ Uniform(0.25, 1.5) Mbit，代入传输时间前转换为 bit；
- C_n/D_n ~ Uniform(300, 1000) cycle/bit；
- eta_n ~ Uniform(1.5, 3.0)；
- L_n^ddl = clip(ceil(eta_n*(D_n/R_ref+C_n/f_ref)/Delta t), 10, 150)；
- t_n^ddl = t_n^arr + L_n^ddl；
...
|---:|---|---|---|---|
| 1 | route | {idle, local, defer} union (U \ {i}) | idle | 1[Q_i^unb != empty] |
| 2 | tx_select | {idle} union (U \ {i}) | idle | 1[exists j: Q_i->j^tx != empty] |
| 3 | resource_group | {idle, 1, 2, 3, 4, 5} | idle | 1[tx_select != idle] |
| 4 | resource_width | {1,2} | 1 | 1[tx_select != idle] 1[resource_group != idle] |
| 5 | power_level | {0, 0.25, 0.5, 1} | 0 | b_width |
| 6 | cpu_queue | {idle} union U | idle | 1[Q_i^loc != empty or exists k != i: Q_k->i^cpu != empty] |
| 7 | cpu_frequency | {0, 0.25, 0.5, 1} | 0 | 1[cpu_queue != idle] |
...
active 通信 proposal 只允许一个组或两个相邻组。resource_group=5 与 resource_width=2 的组合由 mask 排除；策略网络不直接输出任意 RU 子集。resource_group + resource_width 只形成 S_prop，不代表已经执行占用。
...
1. resource_group + resource_width 构造 S_prop；
2. power_level 构造 p_prop = power_level * P_i^max；
3. raw p_prop=0 立即 canonicalize 为 communication idle；
...
6. 按 1.0 -> 0.5 -> 0.25 -> 0 的离散 candidate power 顺序进行 power-first energy downgrade；
7. 必要时按 Section 2 的既有 CPU frequency downgrade 顺序选择最高可行档位；
8. 形成最终 y、x、S_exec、p_exec、CPU frequency、service、actual energy 和 outage。
...
| E2E latency | 对每个 done 任务计算 (t_cmp-t_arr)*Delta t，再汇总 | 只对 completed task；无 completed sample 为 NA |
| energy consumption | sum(E_tx + E_cpu) 的实际主动能耗 | 不包括推进能耗；只计当前 episode service slots |
...
generated_task_count 包括 t=0,...,T-1 槽末实际生成的任务；最后槽生成但未进入 route/service 的任务计入 generated 和 truncated，不计入 expired。E2E latency、outage rate 等有 sample gate 的指标必须同时保存 mean、std 和 valid_sample_count；aggregate 不能把 NA 样本隐式转换成零。
...
训练使用 Section 3 的 active-branch joint log-prob、active-branch entropy、post-service/post-settlement reward 和 contiguous recurrent minibatch。固定 horizon no-bootstrap contract 必须保持：t=T-1、terminated 和 truncated 的 b_boot=0；不得创建虚构 s_T。本文固定 NO BURN-IN；chunk initial hidden 唯一来自 rollout buffer 对应位置保存的 hidden_in，不得重新估计或重建。
